...
flowchart TD
    ...
    S -->|예| U["_search_by_chromadb()<br/>전체 벡터 후보 의미 검색"]
...
- combined 모드에서는 ChromaDB 호출 시 RDB 후보 ID를 넘기지 않아, RDB 후보 안으로 의미 검색을 제한하지 않습니다.
...
| A4 | 제외 | fallback query relevance bonus는 Combined 개선 효과가 제한적이라 최종 제외 |
...
| `_search_by_chromadb()` | `place_service.py` | ChromaDB 의미 검색. combined 모드에서는 RDB 후보 제한 없이 검색 |
