...
> 사용자 질문 → 의도 분류 → 장소추천 핸들러 → LLM 쿼리 파싱/정규화 → RDB 후보 필터링 → 위치 기반 후보 보정 또는 ChromaDB 의미 검색 → 점수 계산 → 후보 20개 성향 리랭킹 → 최종 5개 선택 → 이미지/추천 이유 생성 → 응답 반환
...
flowchart TD
    ...
    K --> L["ParsedQuery에 매핑"]
    L --> M["정규화<br/>요금/용품/실내외/제한사항/장소유형/landmark"]

    I --> N["랜드마크/GPS 좌표 처리"]
    N --> O["_filter_with_relaxation()"]
    O --> P["_filter_by_rdb()<br/>SQL 조건으로 후보 content_id 최대 500개"]
    ...
    S -->|예| U["_search_by_chromadb()<br/>일반 질문: 전체 의미 검색<br/>근처/GPS 질문: 반경 후보 안 재순위"]
    U --> V["_fetch_and_score()<br/>RDB 상세 조회 + final_score 계산"]
    V --> W["district/sub_category/landmark 거리<br/>+ 요금/제한사항 hard filter"]
    T --> Y["검색 결과 places[]"]
    W --> Y

    Y --> Z["_rerank_places_with_profile()<br/>후보 최대 20개 성향 재정렬 → 상위 5개"]
...
5. 기본 검색은 `top_k=5` 대신 최대 20개 후보를 먼저 가져옵니다.
6. `_rerank_places_with_profile()` 로 후보 20개를 프로필 기반 리랭킹한 뒤 상위 5개만 남깁니다.
7. `enrich_place_images()` 로 장소 이미지 URL을 붙입니다.
8. `generate_place_reasons()` 로 장소별 추천 이유를 생성합니다.
9. `_format_place_list_response()` 로 챗봇 본문 텍스트를 만듭니다.
...
| `_normalize_sub_category_intent()` | 애완카페/애견카페/반려견카페 등 alias를 `카페` 계열로 정규화 |
| `_normalize_landmark_intent()` | LLM이 놓친 흔한 landmark를 복구하고, `내 주변`·`주변 카페` 같은 현재 위치 표현을 보정 |
...
`_filter_by_rdb(parsed, db, max_candidates=RDB_FILTER_MAX_CANDIDATES)` 는 `ParsedQuery.objective` 를 SQL 조건으로 변환합니다.

현재 주요 후보 수 상수:

| 상수 | 값 | 의미 |
| --- | ---: | --- |
| `RDB_FILTER_MAX_CANDIDATES` | 500 | RDB 조건 필터로 가져오는 최대 후보 수 |
| `CHROMA_FILTERED_FETCH_N` | 200 | ChromaDB 결과를 후보 ID로 제한할 때 넉넉하게 가져오는 fetch 수 |
...
랜드마크 또는 GPS 좌표가 있으면 `_filter_by_rdb()` 는 거리순으로 후보를 자릅니다. 반경은 `1000m → 2000m → 3000m` 순으로 넓히며 최소 3개 후보를 확보하면 중단합니다.

| 위치 질문 유형 | 처리 방식 |
| --- | --- |
| `경복궁 근처`, `서울역 주변` 등 named landmark | 카카오 API로 landmark 좌표 조회 후 반경 후보 생성, 최종 단계에서 3km 밖 결과 제거 |
| `내 주변`, `내 근처`, `주변 카페 추천` 등 GPS 질문 | 전달된 `user_lat/user_lng` 를 기준으로 반경 후보 생성 |

`_has_proximity_context()` 가 참인 경우 combined 모드에서도 ChromaDB를 전체 자유 검색하지 않고, 먼저 만든 거리 후보 `candidate_ids` 안에서만 재순위합니다. Chroma 결과가 요청 수보다 적으면 같은 반경 후보의 RDB fallback 결과를 중복 없이 보충합니다.
...
1. district 제거
2. sub_category + district 제거

목적은 “조건이 너무 빡세서 아무 장소도 못 찾는 상황”을 줄이는 것입니다. 단, named landmark 질문에서는 `근처` 의미를 잃지 않도록 landmark 자체는 완화하지 않습니다.
...
| `_filter_candidate_ids_by_requested_place_type()` | LLM이 sub_category로 못 잡은 장소 유형을 name/sub_category/description 텍스트로 재필터. 단, 필터 결과가 5개 미만이면 원본 후보를 유지해 과도한 후보 손실을 방지 |
...
3. `candidate_ids` 가 전달된 경우에는 ChromaDB 결과 중 `content_id` 가 후보 목록에 있는 것만 유지
4. 요금 조건이 있으면 후처리 필터를 위해 더 넉넉하게 검색

주의할 점:

- RDB 후보 필터링은 여전히 객관 조건 후보를 만들고 fallback/RDB only 결과에 사용됩니다.
- combined 모드의 일반 질문은 ChromaDB 호출 시 RDB 후보 ID를 넘기지 않아 의미 검색 후보 폭을 넓게 유지합니다.
- 단, landmark/GPS처럼 위치가 핵심인 질문은 거리 후보 안에서만 ChromaDB 재순위를 수행합니다.
- `subjective` 가 비어 있어도 `raw_query` 를 사용해 ChromaDB 의미 검색을 수행합니다.
- 후보 ID 제한 검색이 필요한 경로에서는 `CHROMA_FILTERED_FETCH_N=200` 으로 더 넉넉하게 가져온 뒤 후처리합니다.
...
| `_apply_explicit_objective_guard()` | 사용자가 명시한 district/sub_category 불일치 |
| `_apply_landmark_distance_guard()` | named landmark 기준 최대 반경 밖 장소 |
...
```python
final_score = rule_score + category_bias + query_relevance_bonus + condition_match_bonus
```
...
5차 baseline에서는 Combined와 RDB only가 전 구간 동일하게 나타났습니다. 원인은 `subjective` 가 비어 있을 때 ChromaDB 재순위를 생략하고 RDB fallback으로 조기 반환하던 조건이었습니다.

6차 이후 평가셋과 실서비스 오류 수정 과정에서 다음 변경을 반영했습니다.

| 실험 | 반영 여부 | 내용 |
| --- | --- | --- |
| A1 | 반영 | RDB 후보 수 200 → 500, Chroma 후보 fetch 50 → 200 |
| A2 | 반영 | combined 모드에서 Chroma 후보를 RDB 후보 안으로 제한하지 않음 |
| A3 | 반영 | `subjective` empty 조기 fallback 제거, 원문 query로 Chroma 검색 수행 |
| 7차 | 반영 | RDB fallback 전용 `query_relevance_bonus`, 거리/카테고리/시간 `condition_match_bonus` 추가 |
| A5 | 반영 | 장소 유형 텍스트 필터 결과가 너무 적으면 원본 후보 유지 |
| 8차 | 반영 | 카페 alias 보강, 명시 district/sub_category hard guard 추가 |
| 9~10차 | 반영 | landmark 복구, landmark 거리 guard, 근처/GPS 질문의 거리 후보 우선 재순위 경로 추가 |

최종 결과 기준 Combined Hit@5는 5차 baseline 29.0%에서 6차 final 62.6%로 +33.6%p 개선되었고, Recall@5는 2.6%에서 11.5%로 개선되었습니다. Hit@20도 55.7%에서 84.0%로 상승해 상위 20개 후보 회수 성능도 함께 개선되었습니다.

카테고리별로는 주관 형용사 4.2% → 87.5%, 지역 x 장소유형 12.5% → 56.3%, 위치 기반 근처 20.0% → 53.3%, 시간 조건 27.3% → 63.6%로 개선되었습니다. 가장 큰 개선 요인은 A3이며, A1/A2/A5는 후보 풀과 필터 안정성을 보강하는 역할입니다.

---

## 9. 프로필 리랭킹

`_rerank_places_with_profile()` 은 검색 결과를 받은 뒤 강아지/보호자 태그를 반영합니다. 현재 실서비스는 최종 5개를 바로 재정렬하지 않고, 먼저 검색 후보를 최대 20개까지 확보한 뒤 프로필 점수를 더해 상위 5개를 선택합니다.
...
| 조건 | 보정 |
| --- | --- |
| 강아지 예민도 `e > 3` + 실외 장소 | -0.15 |
| 강아지 예민도 `e > 3` + 반려견놀이터/레포츠 | -0.15 |
...
이 단계는 ChromaDB 검색 자체를 다시 하지 않고, 이미 검색된 후보 최대 20개의 `final_score` 에 프로필 보정값을 더하는 방식입니다. 즉 성향 점수는 후보 생성기가 아니라 최종 노출 순서를 조정하는 리랭커입니다.
...
| `_filter_with_relaxation()` | `place_service.py` | RDB 후보 0건 시 district 중심 조건 완화. named landmark는 유지 |
| `_filter_by_rdb()` | `place_service.py` | objective 조건 기반 SQL 후보 필터. 기본 후보 상한은 500개 |
| `_filter_candidate_ids_by_requested_place_type()` | `place_service.py` | 명시 장소 유형 텍스트 필터. 결과가 5개 미만이면 원본 후보 유지 |
| `_filter_by_time()` | `place_service.py` | 운영시간 조건 필터 |
| `_search_by_chromadb()` | `place_service.py` | ChromaDB 의미 검색. 일반 combined는 자유 검색, 위치 기반 질문은 반경 후보 안 재순위 |
| `_apply_explicit_objective_guard()` | `place_service.py` | 명시 district/sub_category 최종 보장 |
| `_apply_landmark_distance_guard()` | `place_service.py` | named landmark 반경 밖 결과 제거 |
| `_merge_unique_places()` | `place_service.py` | 위치 기반 Chroma 결과 부족 시 fallback 후보를 중복 없이 보충 |
