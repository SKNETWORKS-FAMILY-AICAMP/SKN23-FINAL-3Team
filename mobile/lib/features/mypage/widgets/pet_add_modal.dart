...
/// 마이페이지 [+추가] → 반려견 등록 모달.
///
/// 온보딩 흐름과 분리된 독립 모달. 성공 시 `userPetsProvider` invalidate +
/// `authProvider.refreshUser()` 로 대표 반려견 포함 전체 동기화.
class PetAddModal extends ConsumerStatefulWidget {
  const PetAddModal({super.key});

  @override
  ConsumerState<PetAddModal> createState() => _PetAddModalState();
}

class _PetAddModalState extends ConsumerState<PetAddModal> {
  ...
  Future<void> _save() async {
    ...
    // 인증 상태 확인 — 세션 만료 시 사전 차단
    final auth = ref.read(authProvider);
    if (auth is! AuthAuthenticated) {
      Fluttertoast.showToast(msg: '로그인이 필요합니다. 다시 로그인해주세요.');
    ...
    try {
      ...
      await ref.read(petApiProvider).create(
            PetCreate(
              breedId: _breedId!,
              name: name,
      ...
      if (mounted) {
        Fluttertoast.showToast(msg: '$name 등록됐어요 🐾');
      ...
      Fluttertoast.showToast(
        msg: detail ?? '등록 실패. 잠시 후 다시 시도해주세요.',
      );
    } catch (_) {
      Fluttertoast.showToast(msg: '등록 실패. 잠시 후 다시 시도해주세요.');
    } finally {
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _TappableRow(
                      ...
                      child: Text(
                        _breedName ?? '견종을 선택하세요',
                        style: TextStyle(
                          color: _breedName == null
...
class _ProfilePhotoPicker extends StatelessWidget {
  const _ProfilePhotoPicker({required this.image, required this.onTap});
